...
if (acs_enabled) {
  ...
  if (!is.null(acs_result)) {
    ...
    p_qcat <- ggplot2::ggplot(
      ...
      ggplot2::geom_tile() +
      ggplot2::geom_text(ggplot2::aes(label = qcat_num), size = 2.5) +
      ggplot2::scale_fill_viridis_c(
        name = "qcat\n(higher = more distinctive positive trend)",
        limits = c(1, 7),
        breaks = 1:7,
        option = "viridis"
      ) +
    ...
    p_pdi <- ggplot2::ggplot(
      ...
      ggplot2::geom_tile() +
      ggplot2::geom_text(ggplot2::aes(label = sprintf("%.2f", mean_pdi)), size = 2.3) +
      ggplot2::scale_fill_viridis_c(name = "Mean PDI", option = "viridis") +
