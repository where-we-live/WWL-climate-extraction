...
if (acs_enabled) {
  ...
  if (!is.null(acs_result)) {
    ...
    p_qcat <- ggplot2::ggplot(
      ...
      ggplot2::geom_tile() +
      ggplot2::geom_text(ggplot2::aes(label = qcat_num), size = 3.4, color = "black") +
    ...
    p_pdi <- ggplot2::ggplot(
      ...
      ggplot2::geom_tile() +
      ggplot2::geom_text(ggplot2::aes(label = sprintf("%.2f", mean_pdi)), size = 3, color = "black") +
      ggplot2::scale_fill_viridis_c(
        name = "Mean PDI",
        option = "viridis",
        begin = 0.18
    ...
    grid::grid.newpage()
    grid::grid.text(
      "Trend distinctiveness and climate-perception alignment",
      y = 0.98,
    ...
    lay <- grid::grid.layout(
      nrow = 1,
      ncol = 3,
      widths = grid::unit(c(1, 0.08, 1), "null")
    )
    ...
    print(p_pdi,  vp = grid::viewport(layout.pos.row = 1, layout.pos.col = 3))
    dev.off()
...
plot_metric_timeseries <- function(metric_name, question_code) {
  ...
  trend_df <- dat %>%
    ...
    dplyr::mutate(
      label = sprintf("Slope = %.2f / decade", slope_per_decade),
      x = -Inf,
      y = Inf
    )
  ...
  p <- ggplot(dat, aes(x = wateryear, y = value)) +
    ...
    scale_y_continuous(expand = ggplot2::expansion(mult = c(0.03, 0.18))) +
    ...
    theme(
      ...
      plot.margin = grid::unit(c(8, 6, 6, 6), "pt")
    )
