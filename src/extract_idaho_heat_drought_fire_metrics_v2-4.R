...
if (acs_enabled) {
  ...
  acs_result <- tryCatch({
    ...
    bg <- id_bg %>%
      dplyr::transmute(
        ...
        pov_num = pov_numE,
        pov_rate = dplyr::if_else(!is.na(pov_denomE) & pov_denomE > 0 & !is.na(pov_numE),
                                  pov_numE / pov_denomE,
                                  NA_real_),
    ...
    inter <- suppressWarnings(sf::st_intersection(
      ...
      bg_proj %>% dplyr::select(GEOID, total_pop, medinc, pov_denom, pov_num, pov_rate, edu_denom, bachplus_num, age65_num)
    )) %>%
      dplyr::mutate(
    ...
    acs_covars <- inter %>%
      ...
      dplyr::summarise(
        ...
        poverty_pct = dplyr::if_else(sum(pop_est[is.finite(pov_rate_w)], na.rm = TRUE) > 0,
                                     100 * stats::weighted.mean(pov_rate_w, w = pop_est, na.rm = TRUE),
                                     NA_real_),
  ...
  if (!is.null(acs_result)) {
    ...
    p_heat <- ggplot2::ggplot(
      pdi_question_acs,
      ggplot2::aes(x = question, y = .town, fill = mean_pdi)
    ) +
      ggplot2::geom_tile(color = "white") +
      ggplot2::facet_wrap(~ stressor, scales = "free_x") +
      ggplot2::labs(
        title = "Town by question PDI with Heat, Drought, and Fire panels",
        x = "Question",
        y = "Town",
        fill = "Mean PDI"
      )
    ggplot2::ggsave(file.path(plot_dir, "pdi_heatmap_by_stressor.png"), p_heat, width = 10, height = 6, dpi = 300)
